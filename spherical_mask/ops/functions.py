...
class BallQueryBatchFlat(Function):
    @staticmethod
    def forward(ctx, radius, nsample, xyz, new_xyz, offset, new_offset):
        ...
        ops.ballquery_cuda(m, radius, nsample, xyz, new_xyz, offset, new_offset, idx)

        # print('debug', idx.shape)
        return idx


ballquery_batchflat = BallQueryBatchFlat.apply


class FurthestSamplingBatchFlat(Function):
    @staticmethod
    def forward(ctx, xyz, offset, new_offset):
        ...
        ops.furthestsampling_cuda(b, n_max, xyz, offset, new_offset, tmp, idx)
        del tmp
        return idx


furthestsampling_batchflat = FurthestSamplingBatchFlat.apply


class FurthestSamplingIABatchFlat(Function):
    @staticmethod
    def forward(ctx, xyz, offset, new_offset):
        """
        input: xyz: (n, 6), offset: (b), new_offset: (b)
        output: idx: (m)
        """
        assert xyz.is_contiguous()
        n, b, n_max = xyz.shape[0], offset.shape[0], offset[0]
        ...
        idx = torch.cuda.IntTensor(new_offset[b - 1].item()).zero_()
        tmp = torch.cuda.FloatTensor(n).fill_(1e10)
        ops.furthestsampling_ia_cuda(b, n_max, xyz, offset, new_offset, tmp, idx)
        del tmp
        return idx


furthestsampling_ia_batchflat = FurthestSamplingIABatchFlat.apply
